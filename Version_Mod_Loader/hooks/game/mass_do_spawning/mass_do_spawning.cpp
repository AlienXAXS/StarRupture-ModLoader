...
namespace Hooks::MassDoSpawning
{
	...
	using DoSpawning_t = void(__fastcall*)(void* thisPtr);
	...
	static void __fastcall Detour(void* thisPtr)
	{
		...
		if (!g_beforeCallbacks.empty())
		{
			...
			for (size_t i = 0; i < g_beforeCallbacks.size(); ++i)
			{
				...
				{
					ModLoaderLogger::LogError(L"[MassDoSpawning] Exception in Before callback #%zu: %S", i + 1,
					                          e.what());
				}
				catch (...)
				{
		...
		if (cancelled)
		{
			ModLoaderLogger::LogDebug(
				L"[MassDoSpawning] Spawn batch cancelled — skipping original and After callbacks");
			return;
		}
		...
		if (!g_afterCallbacks.empty())
		{
			...
			for (size_t i = 0; i < g_afterCallbacks.size(); ++i)
			{
				...
				{
					ModLoaderLogger::LogError(L"[MassDoSpawning] Exception in After callback #%zu: %S", i + 1,
					                          e.what());
				}
				catch (...)
				{
	...
	bool Install()
	{
		...
		if (!pattern || pattern[0] == '\0')
		{
			ModLoaderLogger::LogWarn(
				L"[MassDoSpawning] Pattern not set (TODO: fill via IDA/x64dbg) — hook not installed");
			return false;
		}
	...
	void UnregisterBeforeCallback(BeforeCallback callback)
	{
		...
		if (it != g_beforeCallbacks.end())
		{
			...
			ModLoaderLogger::LogDebug(L"[MassDoSpawning] Before callback unregistered (%zu remaining)",
	...
	void UnregisterAfterCallback(AfterCallback callback)
	{
		...
		if (it != g_afterCallbacks.end())
		{
			...
			ModLoaderLogger::LogDebug(L"[MassDoSpawning] After callback unregistered (%zu remaining)",
