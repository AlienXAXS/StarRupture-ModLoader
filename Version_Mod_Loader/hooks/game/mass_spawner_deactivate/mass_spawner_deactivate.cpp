...
namespace Hooks::MassSpawnerDeactivate
{
    ...
    typedef void(__fastcall* DeactivateSpawner_t)(void* thisPtr, bool bPermanently);
    ...
    static void __fastcall Detour(void* thisPtr, bool bPermanently)
    {
        ModLoaderLogger::LogDebug(L"[MassSpawnerDeactivate] DeactivateSpawner called — spawner=%p, permanently=%d",
            thisPtr, (int)bPermanently);

        // Before pass — any callback returning true cancels the operation
        bool cancelled = false;
        if (!g_beforeCallbacks.empty())
        {
            ModLoaderLogger::LogDebug(L"[MassSpawnerDeactivate] Running %zu Before callback(s)", g_beforeCallbacks.size());
            for (size_t i = 0; i < g_beforeCallbacks.size(); ++i)
            {
                ...
                try
                {
                    ...
                    {
                        ModLoaderLogger::LogInfo(L"[MassSpawnerDeactivate] Before callback #%zu cancelled deactivation", i + 1);
                        cancelled = true;
                    }
                }
                catch (const std::exception& e)
                {
                    ModLoaderLogger::LogError(L"[MassSpawnerDeactivate] Exception in Before callback #%zu: %S", i + 1, e.what());
                }
                catch (...)
                {
                    ModLoaderLogger::LogError(L"[MassSpawnerDeactivate] Unknown exception in Before callback #%zu", i + 1);
                }
            }
        }
        ...
        if (cancelled)
        {
            ModLoaderLogger::LogDebug(L"[MassSpawnerDeactivate] Deactivation cancelled — skipping original and After callbacks");
            return;
        }
        ...
        {
            ModLoaderLogger::LogDebug(L"[MassSpawnerDeactivate] Running %zu After callback(s)", g_afterCallbacks.size());
            for (size_t i = 0; i < g_afterCallbacks.size(); ++i)
            {
                ...
                {
                    ModLoaderLogger::LogError(L"[MassSpawnerDeactivate] Exception in After callback #%zu: %S", i + 1, e.what());
                }
                catch (...)
                {
                    ModLoaderLogger::LogError(L"[MassSpawnerDeactivate] Unknown exception in After callback #%zu", i + 1);
                }
            }
        }
    }
    ...
    bool Install()
    {
        ...
        {
            ModLoaderLogger::LogWarn(L"[MassSpawnerDeactivate] Pattern not set (TODO: fill via IDA/x64dbg) — hook not installed");
            return false;
        }
    ...
    void UnregisterBeforeCallback(BeforeCallback callback)
    {
        ...
        if (it != g_beforeCallbacks.end())
        {
            ...
            ModLoaderLogger::LogDebug(L"[MassSpawnerDeactivate] Before callback unregistered (%zu remaining)", g_beforeCallbacks.size());
        }
    }
    ...
    void UnregisterAfterCallback(AfterCallback callback)
    {
        ...
        if (it != g_afterCallbacks.end())
        {
            ...
            ModLoaderLogger::LogDebug(L"[MassSpawnerDeactivate] After callback unregistered (%zu remaining)", g_afterCallbacks.size());
        }
    }
}
