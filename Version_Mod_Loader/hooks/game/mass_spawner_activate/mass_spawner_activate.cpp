...
namespace Hooks::MassSpawnerActivate
{
	...
	using ActivateSpawner_t = void(__fastcall*)(void* thisPtr, bool bDisableAggroLock);
	...
	static void __fastcall Detour(void* thisPtr, bool bDisableAggroLock)
	{
		ModLoaderLogger::LogDebug(L"[MassSpawnerActivate] ActivateSpawner called — spawner=%p, aggroLock=%d",
		                          thisPtr, static_cast<int>(bDisableAggroLock));

		// Before pass — any callback returning true cancels the operation
		bool cancelled = false;
		if (!g_beforeCallbacks.empty())
		{
			ModLoaderLogger::LogDebug(L"[MassSpawnerActivate] Running %zu Before callback(s)",
			...
			for (size_t i = 0; i < g_beforeCallbacks.size(); ++i)
			{
				...
				try
				{
					...
					{
						ModLoaderLogger::LogInfo(L"[MassSpawnerActivate] Before callback #%zu cancelled activation",
						                         i + 1);
						cancelled = true;
					}
				}
				...
				{
					ModLoaderLogger::LogError(L"[MassSpawnerActivate] Exception in Before callback #%zu: %S", i + 1,
					                          e.what());
				}
				catch (...)
				{
					ModLoaderLogger::LogError(L"[MassSpawnerActivate] Unknown exception in Before callback #%zu",
					                          i + 1);
				}
			}
		}
		...
		if (cancelled)
		{
			ModLoaderLogger::LogDebug(
				L"[MassSpawnerActivate] Activation cancelled — skipping original and After callbacks");
			return;
		}
		...
		if (!g_afterCallbacks.empty())
		{
			...
			for (size_t i = 0; i < g_afterCallbacks.size(); ++i)
			{
				...
				{
					ModLoaderLogger::LogError(L"[MassSpawnerActivate] Exception in After callback #%zu: %S", i + 1,
					                          e.what());
				}
				catch (...)
				{
	...
	void RegisterBeforeCallback(BeforeCallback callback)
	{
		...
		ModLoaderLogger::LogDebug(L"[MassSpawnerActivate] Before callback registered (%zu total)",
		                          g_beforeCallbacks.size());
	}
	...
	void UnregisterBeforeCallback(BeforeCallback callback)
	{
		...
		if (it != g_beforeCallbacks.end())
		{
			...
			ModLoaderLogger::LogDebug(L"[MassSpawnerActivate] Before callback unregistered (%zu remaining)",
			                          g_beforeCallbacks.size());
		}
	}
	...
	void RegisterAfterCallback(AfterCallback callback)
	{
		...
		ModLoaderLogger::LogDebug(L"[MassSpawnerActivate] After callback registered (%zu total)",
	...
	void UnregisterAfterCallback(AfterCallback callback)
	{
		...
		if (it != g_afterCallbacks.end())
		{
			...
			ModLoaderLogger::LogDebug(L"[MassSpawnerActivate] After callback unregistered (%zu remaining)",
			                          g_afterCallbacks.size());
		}
	}
}
