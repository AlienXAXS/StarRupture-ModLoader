...
namespace Hooks::MassSpawnerActivate
{
	...
	using BeforeCallback = bool(*)(void* spawner, bool bDisableAggroLock);

	// After: fires after ActivateSpawner completes (only if not cancelled).
	// Spawner is AAbstractMassEnemySpawner* passed as void*.
	using AfterCallback = void(*)(void* spawner, bool bDisableAggroLock);
	...
	// Returns the trampoline address of the original AAbstractMassEnemySpawner::ActivateSpawner,
	// or 0 if not yet installed.
	// Cast to: void(__fastcall*)(void* thisPtr, bool bDisableAggroLock)
