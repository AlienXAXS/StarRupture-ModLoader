	...
	namespace RailScanner {
		...
		void ScanRailSocketState(SDK::UWorld* world)
		{
			...
			for (int32_t i = 0; i < objArray->NumElements; ++i)
			{
				...
				SDK::AActor* startJunction = ReadAt<SDK::AActor*>(base, OFF_ConnectedStartActor);
				SDK::AActor* endJunction   = ReadAt<SDK::AActor*>(base, OFF_ConnectedEndActor);
				SDK::USplineComponent* spline = ReadAt<SDK::USplineComponent*>(base, OFF_Spline);
			...
			if (nRailsTotal == 0)
			{
				...
				else
				{
					...
					for (int32_t i = 0; i < objArray->NumElements; ++i)
					{
						...
						SDK::FVector startPos = ReadAt<SDK::FVector>(
							base, OFF_MS_StartTransform + FTRANSFORM_TRANSLATION_OFFSET);
						SDK::FVector endPos = ReadAt<SDK::FVector>(
						...
						SDK::AActor** spawnerData  = ReadAt<SDK::AActor**>(base, OFF_MS_ConnectingSpawners);
			...
			for (auto& [junction, railEnds] : junctionMap)
			{
				...
				if (!conflictPairs.empty())
				{
					...
					LOG_WARN("RailScanner: Junction %p -- %d rails, %d GEOMETRY conflicts "
						...
						junction, nRails, (int)conflictPairs.size(),
		...
		void OnActorBeginPlay(void* rawActor)
		{
			...
			SDK::AActor* actor = static_cast<SDK::AActor*>(rawActor);
